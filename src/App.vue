...
export default {
  components: {
    ...
    AboutView,
    PrivacyPolicyView,
    CartView, // Add CartView to the components
  },
  setup() {
    const store = useStore();
    store.dispatch('loadCart'); // Load cart from local storage when the app initializes
  },
  data() {
    return {
      isVisible: false, // For scroll-to-top button visibility
    };
  },
  methods: {
    handleLogin() {
  ...
  watch: {
    $route(to) {
      // Check if user is logged in and prevent route to login page if logged in
